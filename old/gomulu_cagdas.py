        ...
        for i in range(4):
            for j in range(4):
                ...
                if shift > 0:
                    ...
                    board[i][j] = 0
                if board[i][j - shift] == board[i][j - shift - 1] and not merged[i][j - shift] and not merged[i][j - shift - 1]:
                    board[i][j - shift - 1] *= 2
...
# spawn pieces random start of every turn
    ...
    for i in range(4):
        for j in range(4):
            ...
            if value > 0:
                ...
                pygame.draw.rect(screen, 'black', [j * 95 + 20, i * 95 + 20, 75, 75], 1, 5)

# check high score and update
def check_high(high_score):
    global init_high
